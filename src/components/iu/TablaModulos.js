      ...
      <tbody>
        {
          componentes.map((est, index) => {
            // metodo para organizar las fechas de la tabla a mas comprensible
            const date = new Date(est.fechaCreacion);
            const creacion = date.getFullYear()+"-"+date.getMonth()+"-"+date.getDay();
            const date1 = new Date(est.fechaActualizacion);
            const fechaActualizacion = date.getFullYear()+"-"+date.getMonth()+"-"+date.getDay();
            ...
            return(
              <tr key={est._id}>
                ...
                <td>
                  ...
                  >
                    <i className="fa-solid fa-highlighter"data={est._id}
